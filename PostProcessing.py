...
class PlotBuilder():
    ...
    def getResults(self,filename='NewResults/RO_opex2.5_powerout50_IRR'):

        self.results = pd.read_csv(f'{filename}.csv',header=None, index_col=False)
    ...
    def dualPlot(self):
        ...
        firstplot=axs[0].scatter(self.results["CAPEX"],self.results["IRR"],c=self.results.LCOE,cmap='viridis')
        axs[0].set_ylabel('IRR (%)')
        axs[0].title.set_text('Sizing Results for Romania in 2021')
        secondplot=axs[1].scatter(self.results["CAPEX"],self.results["NPV"],c=self.results.LCOE,cmap='viridis')
    ...
    def technicalSix(self):
        ...
        secondrow = 0
        location = ['RO', 'Romania']
        self.getResults(filename=f'NewResults/{location[0]}_opex2.5_powerout50_IRR')
        ...
        for each in range(6):
            ...
            cbar = plt.colorbar(aplot, ax=axs[firstrow, secondrow])
            cbar.ax.set_ylabel(f'{factor[each]}', rotation=270, labelpad=10)
